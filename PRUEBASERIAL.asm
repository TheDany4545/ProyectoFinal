...
CONFIG_PWM
    ...
    MOVLW   .120
    MOVWF   PR2			; COLOCO EL VALOR DEL PERIODO DE MI SEÃ?AL 20mS   (PASO#2)
    
    BANKSEL PORTA
    BSF	    CCP2CON, CCP2M3
    BSF	    CCP2CON, CCP2M2
    BSF	    CCP2CON, CCP2M1
    BSF	    CCP2CON, CCP2M0	 ; MODO PWM				  (PASO#3)
    
    MOVLW   B'00011011'
    MOVWF   CCPR2L		    ; MSB   DEL DUTY CICLE			  (PASO#4 Estableciendo ciclo de trabajo)
    BSF	    CCP2CON, DC2B0
    BSF	    CCP2CON, DC2B1	    ; LSB del duty cicle
    
    BCF	    PIR1, TMR2IF							 ;(PASO#5 Borrando bandera del TMR2 / Config. e Inicializar TMR2)
    
    BSF	    T2CON, T2CKPS1
    BSF	    T2CON, T2CKPS0	    ; PRESCALER 1:16				  (PASO#5.2)
    
    BSF	    T2CON, TMR2ON	    ; HABILITAMOS EL TMR2			  (PASO#5.3)
    BTFSS   PIR1, TMR2IF
    GOTO    $-1
    BCF	    PIR1, TMR2IF
    
    BANKSEL TRISC
    BCF	    TRISC, RC1		    ; RC1 / CCP2 SALIDA PWM			  (PASO#6 Habilitamos la salida del PWM espués de que haya un nuevo ciclo empezado)
    RETURN
